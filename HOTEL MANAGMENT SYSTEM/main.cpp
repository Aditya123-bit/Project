...
class HOTEL {
private:
    // storing room and customer details
    int room_no;
    char name[30];
    char address[50];
    char phone[15];
    long days;      // Number of days customer is staying
    long cost;      // Total cost for stay
    char rtype[30]; // Room type
    long pay;       // Food charges

    // functions to assign food costs
    void breakfast(int);
    void lunch(int);
    void dinner(int);
...
public:
    ...
    void display();         // Display specific customer details
    void rooms();           // Display all allotted rooms
    void edit();            // Edit customer details or checkout
    int check(int);         // Check room availability
    void modify();          // Modify customer information
    void delete_rec();      // Check out a customer
    void restaurant();      // Food order from restaurant
    void modify_name(int);  // Modify customer's name
    void modify_address(int); // Modify customer's address
    void modify_phone(int); // Modify customer's phone
    void modify_days(int);  // Modify customer's stay duration
} h;



// Main menu function to handle user navigation
...
void HOTEL::add() {
    system("clear");
    int r, flag;
    ofstream fout("Record.DAT", ios::app | ios::binary); // Open file in append mode
    cout << "\n\t\t\t +-----------------------+";
    cout << "\n\t\t\t | Rooms  |   Room Type  |";
    cout << "\n\t\t\t +-----------------------+";
    cout << "\n\t\t\t |   1-50 |    Deluxe    |";
    ...
    flag = check(r); // Check room status
    if (flag == 1)
        cout << "\n Sorry, Room is already booked.\n";
    else if (flag == 2)
        cout << "\n Sorry, Room does not exist.\n";
    else {
        // Assign room and customer details
        room_no = r;
        cout << " Name: ";
        cin >> name;
        cout << " Address: ";
        ...
        fout.write((char*)this, sizeof(HOTEL)); // Write details to file
...
// More comments added throughout the code for better readability and explanation

// Function to display details of a specific customer
void HOTEL::display() {
    system("clear");
    ifstream fin("Record.DAT", ios::in | ios::binary); // Open file in read mode
    int r, flag = 0;
